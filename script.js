...
var SLOTS = {
  lucky: {
    ...
    btnClass: 'lucky-spin',
    btnLabel: '🍵 SPIN',
    symbols: ['jackpottea.webp','megatea.webp','bigtea.webp','gaivantea.webp','dapao.webp','gaba.webp','puerchik.webp'],
    weights:  [  2,   4,   8,  12,  16,  18,  20],
    paytable: [
      { type: 3, s: ['jackpottea.webp','jackpottea.webp','jackpottea.webp'], mult: 50,  name: 'Джекпот — 3 символа джекпота' },
      { type: 3, s: ['megatea.webp','megatea.webp','megatea.webp'],          mult: 25,  name: 'Три чайных духа Будды' },
      { type: 3, s: ['bigtea.webp','bigtea.webp','bigtea.webp'],             mult: 15,  name: 'Три чайных духа жабы' },
      { type: 3, s: ['gaivantea.webp','gaivantea.webp','gaivantea.webp'],    mult: 10,  name: 'Три гайвани' },
      { type: 3, s: ['dapao.webp','dapao.webp','dapao.webp'],                mult: 6,   name: 'Три Да Хун Пао' },
      { type: 3, s: ['gaba.webp','gaba.webp','gaba.webp'],                   mult: 4,   name: 'Три Габы' },
      { type: 3, s: ['puerchik.webp','puerchik.webp','puerchik.webp'],       mult: 3,   name: 'Три пуэра' },
      { type: 2, s: ['puerchik.webp','puerchik.webp'],                       mult: 1.5, name: 'Два пуэра' },
      { type: 1, s: ['puerchik.webp'],                                        mult: 0.5, name: 'Пуэр' },
    ]
  },
  pyramid: {
    title: 'Olympus Lite',
    ...
    btnLabel: '⚡ SPIN',
    symbols: ['Jackpotina.webp','Rare1.webp','Rare2.webp','Rare3.webp','ordinary1.webp','ordinary2.webp','ordinary3.webp'],
    weights:  [  2,              6,           6,           6,           18,              18,              18],
    paytable: [
      { type: 3, s: ['Jackpotina.webp','Jackpotina.webp','Jackpotina.webp'], mult: 50,  name: 'ДЖЕКПОТ + 15 фриспинов!', freespins: 15 },
      { type: 3, s: ['Rare1.webp','Rare1.webp','Rare1.webp'],                mult: 20,  name: 'Три редких I' },
      { type: 3, s: ['Rare2.webp','Rare2.webp','Rare2.webp'],                mult: 20,  name: 'Три редких II' },
      { type: 3, s: ['Rare3.webp','Rare3.webp','Rare3.webp'],                mult: 20,  name: 'Три редких III' },
      { type: 3, s: ['ordinary1.webp','ordinary1.webp','ordinary1.webp'],    mult: 5,   name: 'Три обычных I' },
      { type: 3, s: ['ordinary2.webp','ordinary2.webp','ordinary2.webp'],    mult: 5,   name: 'Три обычных II' },
      { type: 3, s: ['ordinary3.webp','ordinary3.webp','ordinary3.webp'],    mult: 5,   name: 'Три обычных III' },
      { type: 2, s: ['ordinary3.webp','ordinary3.webp'],                     mult: 1.5, name: 'Два обычных' },
      { type: 1, s: ['ordinary3.webp'],                                       mult: 0.5, name: 'Обычный' },
    ]
  }
};
...
function openSlot(type) {
  ...
  // Olympus тема
  var isOlympus = (type === 'pyramid');
  document.getElementById('olympus-bg').classList.toggle('hidden', !isOlympus);
  document.getElementById('olympus-banner').classList.toggle('hidden', !isOlympus);
  document.getElementById('slot-header').className = isOlympus ? 'slot-header screen-header olympus-header' : 'slot-header screen-header';
  // olympus-slot класс добавляем после showScreen
  var screenEl = document.getElementById('screen-slot');
  if (isOlympus) screenEl.classList.add('olympus-slot');
  else screenEl.classList.remove('olympus-slot');
  if (isOlympus) startLightning();

  // Кнопка покупки фриспинов — только для Olympus Lite
  var buyBtn = document.getElementById('buy-fs-btn');
  if (type === 'pyramid') {
    buyBtn.classList.remove('hidden');
    buyBtn.textContent = '⚡ Купить ФС';
  } else {
    buyBtn.classList.add('hidden');
  }
  // Скрываем баннер фриспинов при открытии
  ...
  for (var i = 0; i < 3; i++) {
    ...
    r.innerHTML = symHTML(cfg.symbols[i % cfg.symbols.length]);
...
function buildPaytable(cfg) {
  var isTea = (cfg === SLOTS.lucky);
  var title = isTea ? '🍵 茶 · ТАБЛИЦА ВЫПЛАТ · 茶 🍵' : '◆ ТАБЛИЦА ВЫПЛАТ ◆';
  var html = '<div class="paytable-title' + (isTea ? ' tea-title' : '') + '">' + title + '</div>';
  cfg.paytable.forEach(function(row) {
    var syms;
    if (row.type === 3) syms = row.s.map(symHTML).join(' ');
    else if (row.type === 2) syms = row.s.map(symHTML).join(' ') + ' ❓';
    else syms = symHTML(row.s[0]) + ' ❓ ❓';
...
function symHTML(sym) {
  if (sym && sym.match(/\.(png|webp|jpg|jpeg)$/i)) {
    // Каждое изображение помещаем в квадратный контейнер одинакового размера
    return '<span class="reel-sym-wrap"><img src="' + sym + '" class="reel-sym-img" alt=""></span>';
  }
  return sym;
}
...
function doSpin() {
  ...
  [500, 820, 1140].forEach(function(delay, i) {
    var iv = setInterval(function() {
      reelEls[i].innerHTML = symHTML(cfg.symbols[Math.floor(Math.random() * cfg.symbols.length)]);
    }, 75);
    setTimeout(function() {
      clearInterval(iv);
      reelEls[i].classList.remove('spinning');
      reelEls[i].innerHTML = symHTML(results[i]);
      haptic('light');
      if (i === 2) {
...
function evalResult(res, cfg) {
  var winAmt = 0, winName = '', isJackpot = false, freeSpinsAwarded = 0;
  ...
  for (var i = 0; i < cfg.paytable.length; i++) {
    ...
    if (match) {
      ...
      if (row.freespins) freeSpinsAwarded = row.freespins;
      break;
    }
  }
  ...
  if (winAmt > 0) {
    ...
    if (isJackpot) {
      ...
      if (freeSpinsAwarded > 0) {
        showWinOverlay('⚡', 'ДЖЕКПОТ + ' + freeSpinsAwarded + ' ФРИСПИНОВ!', '+' + fmtWin(winAmt));
        rm.textContent = '⚡ ' + winName + ' +' + fmtWin(winAmt);
        rm.className = 'result-msg jackpot';
        spawnConfetti();
        hapticNotify('success');
        setTimeout(function(){ startFreeSpins(freeSpinsAwarded); }, 3200);
      } else {
...
var FREE_SPIN_LUCK = 5.0; // сильно повышенная удача во время фриспинов
var fsCostPaid = 0;
var fsWonTotal = 0;
var fsGuaranteedMin = 0;
var fsGuaranteedMax = 0;
var fsSpinsTotal = 0;
var fsSpinsDone = 0;
...
function startFreeSpins(count) {
  ...
  fsWonTotal = 0;
  if (fsCostPaid > 0) {
    // Гарантия: 60-120x от ставки (при ставке $1 = $60-$120, при $10 = $600-$1200)
    fsGuaranteedMin = currentBet * 60;
    fsGuaranteedMax = currentBet * 120;
  } else {
    fsGuaranteedMin = 0;
    fsGuaranteedMax = 0;
  }
  updateFreeSpinUI();
  showToast('⚡ ' + count + ' фриспинов активированы!');
  document.getElementById('fs-banner').classList.remove('hidden');
  setTimeout(function(){ runNextFreeSpin(); }, 800);
}
...
function doFreeSpinRound() {
  ...
  document.getElementById('win-line').classList.remove('show');
  document.getElementById('win-overlay').classList.remove('show');
  document.getElementById('result-msg').textContent = '⚡ Фриспин ' + fsSpinsDone + '!';
  ...
  // Повышенная удача:
  // idx 0 = Jackpot (редко — даёт +1 спин, не форсируем)
  // idx 1-3 = Rare (очень часто)
  // idx 4-6 = Ordinary (реже чем обычно)
  var boostedWeights = cfg.weights.map(function(w, idx) {
    if (idx === 0) return w * 1.2;        // Jackpot — чуть чаще для +1 спина
    if (idx >= 1 && idx <= 3) return w * FREE_SPIN_LUCK; // Rare — намного чаще
    return w * 0.4;                        // Ordinary — значительно реже
  });

  var isLastSpin = (freeSpinsLeft === 0);
  var results;
  if (isLastSpin && fsCostPaid > 0 && fsWonTotal < fsGuaranteedMin) {
    results = forceGuaranteedResult(cfg);
  } else {
    results = [
      weightedRand(cfg.symbols, boostedWeights),
      weightedRand(cfg.symbols, boostedWeights),
      weightedRand(cfg.symbols, boostedWeights)
    ];
  }
...
function forceGuaranteedResult(cfg) {
  var needed = fsGuaranteedMin - fsWonTotal;
  var jackpotSym = cfg.symbols[0]; // никогда не выдаём джекпот принудительно

  // Собираем все строки таблицы тип=3 кроме джекпота
  var candidates = [];
  for (var i = 0; i < cfg.paytable.length; i++) {
    var row = cfg.paytable[i];
    if (row.type !== 3) continue;
    if (row.s[0] === jackpotSym) continue; // пропускаем джекпот
    candidates.push(row);
  }

  // Ищем минимальную строку которая покрывает needed
  var best = null;
  for (var j = 0; j < candidates.length; j++) {
    var payout = currentBet * candidates[j].mult;
    if (payout >= needed) {
      if (!best || payout < currentBet * best.mult) best = candidates[j];
    }
  }

  // Если ни одна не покрывает — берём самую дорогую (кроме джекпота)
  if (!best && candidates.length > 0) {
    best = candidates[0]; // первая после джекпота — самая дорогая rare
  }

  // Добавляем рандом в рамках гарантийного диапазона
  // Иногда берём чуть дороже для рандомности 60-120%
  var targetMin = needed;
  var targetMax = fsGuaranteedMax - fsWonTotal;
  var betterCandidates = candidates.filter(function(r){
    var p = currentBet * r.mult;
    return p >= targetMin && p <= targetMax;
  });
  if (betterCandidates.length > 0) {
    best = betterCandidates[Math.floor(Math.random() * betterCandidates.length)];
  }

  if (!best) best = candidates[0];
  return [best.s[0], best.s[1], best.s[2]];
}

function evalFreeSpin(res, cfg) {
  var winAmt = 0, winName = '', isJackpot = false;
  var jackpotSym = cfg.symbols[0];

  for (var i = 0; i < cfg.paytable.length; i++) {
    var row = cfg.paytable[i];
    var match = false;
    if (row.type === 3) match = (res[0]===row.s[0] && res[1]===row.s[1] && res[2]===row.s[2]);
    else if (row.type === 2) match = (res[0]===row.s[0] && res[1]===row.s[1]);
    else match = (res[0]===row.s[0]);
    if (match) {
      winAmt  = currentBet * row.mult;
      winName = row.name;
      if (row.mult >= 25) isJackpot = true;
  ...
  // +1 спин за каждый символ джекпота (не тройной)
  var jackpotCount = res.filter(function(s){ return s === jackpotSym; }).length;
  if (jackpotCount > 0 && !isJackpot) {
    freeSpinsLeft += jackpotCount;
    fsSpinsTotal  += jackpotCount;
    updateFreeSpinUI();
    showToast('⚡ +' + jackpotCount + ' фриспин' + (jackpotCount > 1 ? 'а' : '') + ' за символ джекпота!');
  }
  ...
  if (winAmt > 0) {
    ...
    sfxLose();
    rm.textContent = '⚡ Не выпало...';
    rm.className = 'result-msg lose';
  }

  isSpinning = false;
  if (freeSpinsLeft > 0) {
    setTimeout(function(){ runNextFreeSpin(); }, 1800);
  } else {
    setTimeout(function(){ endFreeSpins(); }, 1500);
  }
}
...
function showFsResult() {
  var modal = document.getElementById('fs-result-modal');
  if (!modal) return;
  var profit = fsWonTotal - fsCostPaid;
  document.getElementById('fs-result-amount').textContent = fmtWin(fsWonTotal);
  document.getElementById('fs-result-cost').textContent = fsCostPaid > 0 ? 'Потрачено: ' + fmtWin(fsCostPaid) : 'Джекпот-фриспины (бесплатно)';
  var profitEl = document.getElementById('fs-result-profit');
  profitEl.textContent = profit >= 0 ? '+' + fmtWin(profit) : fmtWin(profit);
  profitEl.style.color = profit >= 0 ? '#4ade80' : '#f87171';
  modal.classList.remove('hidden');
  spawnConfetti();
}

function closeFsResult() {
  document.getElementById('fs-result-modal').classList.add('hidden');
  fsCostPaid = 0;
  fsWonTotal = 0;
}

function updateFreeSpinUI() {
  var el = document.getElementById('fs-counter');
  if (el) el.textContent = freeSpinsLeft;
  var banner = document.getElementById('fs-banner');
  if (banner) {
    if (freeSpinsLeft > 0) banner.classList.remove('hidden');
    else banner.classList.add('hidden');
  }
}

function startLightning() {
  var el = document.getElementById('olympus-lightning');
  if (!el) return;
  function flash() {
    if (currentSlot !== 'pyramid') return;
...
function buyFreeSpins() {
  if (currentSlot !== 'pyramid') return;
  var cost = currentBet * 100;
  if (balance < cost) {
    showToast('❌ Недостаточно средств! Нужно ' + fmtWin(cost));
    hapticNotify('error');
    return;
  }
  document.getElementById('fs-modal-price').textContent = fmtWin(cost);
...
function confirmBuyFs() {
  ...
  saveBalance();
  updateBalanceUI('lose-flash');
  showToast('⚡ Куплено 15 фриспинов за ' + fmtWin(cost) + '!');
  setTimeout(function(){ startFreeSpins(15); }, 600);
}
